...
export const addCollectionAndDocuments = async <T extends ObjectsToAdd>(
  ...
  try {
    ...
    console.log("Sucessfully added the collection");
  } catch (error) {
    console.log("error creating the collection", error);
  }
};
...
export const getCategoriesAndDocuments = async (): Promise<Category[]> => {
  ...
  const querySnapshot = await getDocs(q); // fetch the data
...
export type AddditionalInformation = {
  displayName?: string;
};
...
export const createUserDocumentFromAuth = async (
  userAuth: User,
  additionalInformation = {} as AddditionalInformation
